...
export function ChatInterface({ onGenerate, isGenerating, onNewObject, userId, glyphConnected, glyphVerified, onConnectGlyph, onVerifyGlyph, trendingKeywords }: ChatInterfaceProps) {
  ...
  const canSubmit = isDemo ? (glyphConnected && glyphVerified && !demoGenerationsExhausted) : !!apiKey;
                ...
                <div className="flex items-start gap-2 bg-[var(--primary)]/10 border border-[var(--primary)]/20 rounded-xl px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] mt-0.5 font-mono leading-relaxed">
                      5 object builds with 5 refinements each. Powered by Claude Sonnet. Requires wallet connection.
                    </p>
                  </div>
                </div>
              )}
          ...
          <input
            ...
            placeholder={
              demoNeedsGlyph
                ...
                : isDemo && glyphConnected && !glyphVerified
                  ? "Sign & continue to verify your identity..."
